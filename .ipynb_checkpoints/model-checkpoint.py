...
class MetaCNN(nn.Module):
    def __init__(self, input_shape, channels, kernels, dilations):
        ...
        self.fc1 = nn.Linear(np.prod(self.last_conv_layer_shape, dtype=np.int32), 32)
        self.fc2 = nn.Linear(32, input_shape[0])
        
        # fc2 outputs the predicted sequence of DTS values for that window. 
        # now take the whole picture with the prediction to predict the single value:
        
        self.BiggerConvNet = BasicConvNet(
    ...
    def forward(self, x):
        ...
        fc2_out = self.fc2(fc1_out)
        sequence_prediction = self.relu(fc2_out)
        sequence_prediction = sequence_prediction.unsqueeze(dim=1)
        sequence_prediction = sequence_prediction.unsqueeze(dim=-1)
        print("sequence_prediction.size()", sequence_prediction.size())
        print("x.size()", x.size())
        # now use the sequence prediction and feed to the conv again:
        combined = torch.cat((x, sequence_prediction), dim=-1) # adding to the column dimension
        print(combined.size())
        combined = combined.permute(0, 2, 3, 1)
        pointwise_prediction = self.BiggerConvNet(combined)
        
        
        return sequence_prediction, pointwise_prediction
